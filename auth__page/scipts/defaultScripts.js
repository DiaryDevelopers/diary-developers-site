...
const inputFiledLogin = document.querySelector("#loginFiled");
const inputFiledPassword = document.querySelector("#passswordField");

const clickAuth =() => {

    const circlePosition1 = document.querySelector(".circle2");

    const circlePositionAnimation1 = () => {
        circlePosition1.style.animation = "authCircleAnimation 5s infinite ease";
    }
...
/* Помощь для связи с Back-End'ом */

let isFiledLoginEmpty;
let isFiledPasswordEmpty;

    const checkFiledLogin = () => {

        if (inputFiledLogin.value === "") {
            inputFiledLogin.style.borderColor = "#CD5C5C"
            isFiledLoginEmpty = true;
    }
        else if (inputFiledLogin.value !== "") {
        inputFiledLogin.style.borderColor = "var(--main--border)"
            isFiledLoginEmpty = false;
    }
        else {
        inputFiledLogin.style.borderColor = "#CD5C5C"
            console.log("inputFiledLogin -> field error 103")
            isFiledLoginEmpty = null;
    }

        console.log(isFiledLoginEmpty)

}
    const checkFiledPassword = () => {

    if (inputFiledPassword.value === "") {
        inputFiledPassword.style.borderColor = "#CD5C5C"
        isFiledPasswordEmpty = true;
    }
    else if (inputFiledPassword.value !== "") {
        inputFiledPassword.style.borderColor = "var(--main--border)"
        isFiledPasswordEmpty = false;
    }
    else {
        inputFiledPassword.style.borderColor = "#CD5C5C"
        console.log("inputFiledPassword -> field error 104")
        isFiledPasswordEmpty = null;
    }

        console.log(isFiledPasswordEmpty)

}

authButton.addEventListener("click", checkFiledLogin);
authButton.addEventListener("click", checkFiledPassword);
